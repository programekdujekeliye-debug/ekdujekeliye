  ...
  return (
    <div
      ...
      className={`group relative p-3 rounded-2xl flex items-center gap-3 cursor-pointer transition-all select-none ${
        isSelected
          ? 'bg-rose-50/90 text-stone-900 shadow-xs border border-rose-200/80 ring-1 ring-rose-500/20'
          : 'hover:bg-stone-100/80 text-stone-700 border border-transparent'
      }`}
    >
      ...
      <div className="relative flex-shrink-0">
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs transition-transform group-hover:scale-105 ${
            conversation.unreadCount > 0
              ? 'bg-gradient-to-br from-rose-700 to-rose-900 text-white shadow-xs'
              : isSelected
        ...
        <span
          className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${
            isWindowActive ? 'bg-emerald-500 ring-2 ring-emerald-400/30' : 'bg-stone-300'
          }`}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1 mb-0.5">
          <span className="font-extrabold text-xs text-stone-900 truncate tracking-tight">
            {conversation.customerName}
          </span>
          <span className="text-[10px] text-stone-400 font-medium whitespace-nowrap font-mono flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-stone-500">
          {conversation.inquiryId && (
            <span className="inline-block px-1.5 py-0.2 rounded bg-stone-200/70 text-stone-800 font-mono font-bold text-[9px] flex-shrink-0">
              {conversation.inquiryId}
            </span>
          )}
          <span className="truncate text-stone-500 text-[11px]">
            {conversation.lastMessageDirection === 'OUTBOUND' && (
              <span className="text-stone-400 mr-1 font-bold">You:</span>
            )}
      ...
      {/* Unread Pill Count */}
      {conversation.unreadCount > 0 && (
        <span className="min-w-[18px] h-[18px] px-1 bg-rose-600 text-white rounded-full font-black text-[10px] flex items-center justify-center shadow-xs flex-shrink-0 animate-pulse">
